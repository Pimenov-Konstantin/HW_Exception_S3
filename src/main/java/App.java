...
public class App {
    public static void start() {
        System.out.println("Введите ваши данные (фио, дату рождения, телефон, пол) через пробел, желательно латиницей:");
    ...
    private static char setSex(String[] data) {
        ...
        if (sex != 'f' && sex != 'm') throw new ConvertSexException("Пол обозначается одной буквой");
        return sex;
    }
    private static long setPhone(String[] data) {
        ...
        if (phoneNumber < 0) throw new ConvertPhoneNumberException("Номер телефона вводится только цифрами");
        return phoneNumber;
    }
    ...
    private static LocalDate setBirthday(String[] data) {
        ...
        if (localDate == null)
            throw new ConvertDateException("Введите дату как в примере dd.mm.yyyy", "SOME TEXT", 0);
        return localDate;
    }
    ...
    private static void writeToFile(User user) throws IOException {
        String path = "src\\main\\java" + user.getLastName() + ".txt";
